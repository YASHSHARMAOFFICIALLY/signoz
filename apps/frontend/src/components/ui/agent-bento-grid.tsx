...
/* ──────────────────────────────────────────────────────
   Niche: AI SRE Observability Copilot
...
const NODES: NodeConfig[] = [
  { id: 'A', x: 50, y: 120, icon: ChatCircle, label: "ALERT", type: 'box' },
  { id: 'Router', x: 125, y: 120, type: 'circle' },
  { id: 'C', x: 200, y: 120, icon: Brain, label: "COPILOT", type: 'box' },
  { id: 'B', x: 280, y: 50, icon: Database, label: "TRACES", type: 'box' },
  { id: 'D', x: 280, y: 190, icon: TerminalWindow, label: "QUERIES", type: 'box' },
];
  ...
  return (
    <div className="w-full h-full flex flex-col gap-3.5 justify-between">
      ...
      <div className="flex gap-4 pt-[0.625rem] pr-[0.625rem] pb-0.5 pl-0.5">
        {[
          { label: "Spans/min", value: "1.2M", trend: "+8%" },
          { label: "MTTR", value: "4.2m", trend: "-38%" },
        ].map((s, i) => {
...
export function Card3() {
  const logs = [
    { agent: "Detector", action: "Checkout p99 breached 2.5s threshold", status: "done", t: "0.2s" },
    { agent: "Correlator", action: "Linked 1.2M spans to deploy #482", status: "done", t: "1.4s" },
    { agent: "Analyzer", action: "Isolating slow DB query…", status: "running", t: "3.1s" },
    { agent: "Remediator", action: "Awaiting root cause from Analyzer", status: "waiting", t: "—" },
    { agent: "Reporter", action: "Idle — queued", status: "idle", t: "—" },
  ];
...
const NS_COLORS: Record<string, { bar: string; dot: string; badge: string; buttonBg: string; buttonBorder: string }> = {
  traces: { bar: "from-violet-500 to-violet-400", dot: "bg-violet-500", badge: "bg-violet-500/15 text-violet-400", buttonBg: "bg-violet-500", buttonBorder: "border-violet-600" },
  logs: { bar: "from-sky-500    to-sky-400", dot: "bg-sky-500", badge: "bg-sky-500/15 text-sky-400", buttonBg: "bg-sky-500", buttonBorder: "border-sky-600" },
  metrics: { bar: "from-emerald-500 to-emerald-400", dot: "bg-emerald-500", badge: "bg-emerald-500/15 text-emerald-400", buttonBg: "bg-emerald-500", buttonBorder: "border-emerald-600" },
  alerts: { bar: "from-amber-500  to-amber-400", dot: "bg-amber-500", badge: "bg-amber-500/15 text-amber-400", buttonBg: "bg-amber-500", buttonBorder: "border-amber-600" },
};

const RETRIEVAL_QUERIES = [
  { ns: "traces", q: "checkout p99 latency spans", t: "0.2s" },
  { ns: "logs", q: "payment service 5xx errors", t: "1.1s" },
  { ns: "traces", q: "db connection pool saturation", t: "2.4s" },
  { ns: "metrics", q: "cpu throttling — cart-svc", t: "4.0s" },
  { ns: "alerts", q: "SLO burn rate — checkout", t: "5.8s" },
  { ns: "logs", q: "deploy #482 error signature", t: "7.2s" },
];

export function Card4() {
  const namespaces = [
    { name: "traces", hits: 342, fill: 88 },
    { name: "logs", hits: 218, fill: 56 },
    { name: "metrics", hits: 97, fill: 25 },
    { name: "alerts", hits: 54, fill: 14 },
  ];
  ...
  return (
    <div className="w-full h-full flex gap-5 py-2 px-3">
      ...
      <div className="flex-1 flex flex-col gap-0 min-w-0 pr-2">
        <p className="text-[8px] font-mono uppercase tracking-widest text-muted-foreground mb-3">Signals</p>
...
export function Card5() {
  const tools = [
    { name: "trace_query", calls: 14, icon: Globe, latency: "280ms", color: "bg-gradient-to-b from-sky-400 to-sky-600", borderColor: "border-sky-600" },
    { name: "log_search", calls: 8, icon: TerminalWindow, latency: "1.2s", color: "bg-gradient-to-b from-emerald-400 to-emerald-600", borderColor: "border-emerald-600" },
    { name: "metric_scan", calls: 22, icon: FileText, latency: "12ms", color: "bg-gradient-to-b from-amber-400 to-amber-600", borderColor: "border-amber-600" },
    { name: "span_analyze", calls: 31, icon: Brain, latency: "95ms", color: "bg-gradient-to-b from-violet-400 to-violet-600", borderColor: "border-violet-600" },
  ];
...
const CARDS = [
  {
    ...
    description: "Watch alerts flow from detection to root cause in real time.",
    visual: <Card1 />,
  ...
  },
  {
    ...
    description: "Track span throughput and mean time to resolution across services.",
    visual: <Card2 />,
    colSpan: "lg:col-span-1",
    height: "h-[260px]",
  },
  {
    ...
    description: "Live log of copilot actions, queries, and trace correlations.",
    visual: <Card3 />,
    colSpan: "lg:col-span-1",
    height: "h-[260px]",
  },
  {
    ...
    description: "Semantic search across traces, logs, metrics, and alerts.",
    visual: <Card4 />,
  ...
  },
  {
    ...
    description: "Monitor query volume, latency, and success rates across SigNoz.",
    visual: <Card5 />,
    colSpan: "lg:col-span-1",
    height: "h-[260px]",
  }
